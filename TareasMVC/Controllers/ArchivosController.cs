...
namespace TareasMVC.Controllers
{
    ...
    public class ArchivosController : ControllerBase
    {
        ...
        public async Task<IActionResult> Ordenar(int tareaId, [FromBody] Guid[] ids)
        {
            ...
            var archivosAdjuntos = await context.ArchivosAdjuntos.Where(x => x.TareaId == tareaId).ToListAsync();

            var archivosAdjuntosIds = archivosAdjuntos.Select(x => x.Id);

            var idsArchivosNoPertenecenALaTarea = ids.Except(archivosAdjuntosIds).ToList();
            ...
            var archivosDiccionario = archivosAdjuntos.ToDictionary(a => a.Id);

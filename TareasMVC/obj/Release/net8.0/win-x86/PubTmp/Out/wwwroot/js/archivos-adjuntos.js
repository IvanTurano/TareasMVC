const inputArchivoTarea = document.getElementById('archivoATarea');
...
async function manejarFocusoutTituloArchivoAdjunto(archivoAdjunto) {
    ...
    if (!archivoAdjunto.titulo()) {
        archivoAdjunto.titulo(tituloAnteriorArchivoAdjunto);
    }

    if (archivoAdjunto.titulo() === tituloAnteriorArchivoAdjunto) {
        return;
    }
...
async function actualizarOrdenArchivosAdjuntos() {
...
$(function () {
    $("#reordenable-archivos").sortable({
        axis: 'y',
        stop: async function () {
            await actualizarOrdenArchivosAdjuntos();
        }
    })
})